              ...
              <div class="row">
                <div class="col-md-12">
                  <div class="box">
                    ...
                    <div class="box-body">
                      <div class="container-fluid">
                        <table id="listContratistas" class="table-light table-bordered table-striped table-hover"  width="98%" cellspacing="0">
                          <thead>
                            <tr>
                              <th>NOMBRE</th>
    ...
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        <div class="modal-header header-modal-sany" style="background-color: #6F42C1">
          <div class="container-fluid">
            <h4 class="modal-title " style="text-align: center; color: black">REGISTRAR TIEMPOS</h4>
          </div>
        </div>

        <div class="modal-body modal-xl">
          <div class="content">
            <form method="POST" name="registrarContratista" id="registrarContratista" onsubmit="return enviarRegistroContratista(event);" action="procesarContratista.php?accion=crear">
              <div class="row">
                ...
                <div class="form-group col-md-12">
                  <label for="recipient-name" class="form-control-label">Codigo Identificacion</label>
                  <input  type="text" class="form-control" name="codigoIdentificacion" id="codigoIdentificacion" minlength="1" maxlength="100" autocomplete="off"/>
                </div>
              </div>
        ...
        <div class="modal-header header-modal-sany" style="background-color: #6F42C1">
          <div class="row">
            <div class="form-group col-md-12 text-center" style="margin-bottom: -8px;background-color: #6F42C1">
              <h4 class="modal-title">MODIFICAR DATOS DE LOS TIEMPOS</h4>
            </div>
          </div>
        </div>
...
<script type="text/javascript">
  ...
  ?>
  <?php  echo cargarDataTables('listContratistas', 'responseContratistas'); ?>
  <?php 
  if(isset($_GET['r'])) {
  ...
  ?>
  if(resquest == "success"){
    swal("Excelente!", "Contratista almacenado correctamente!", "success");
  }else if(resquest == "editado"){
    swal("Excelente!", "Contratista editado correctamente!", "success");
  }else if(resquest == "eliminado"){
    swal("Excelente!", "Contratista eliminado correctamente!", "success");
  ...
  function fnEditarContratista(idUsuario) {
    $.ajax({
      url: "formEditarContratista.php",
      type: "POST",
      dataType: "html",
      data: {idu: idUsuario},
 ...
 function fnEliminarContratista(idUsuario){
  swal({
    title: "Estas seguro que deseas eliminar este contratista?",
    icon: "warning",
    buttons: true,
    dangerMode: true,
  })
  .then((willDelete) => {
    if (willDelete) {
      ...
      window.location="procesarContratista.php?accion=eliminar&id="+idUsuario;
    } 
  });
}
</script>
